...
import { Warehouse, AlertTriangle, CheckCircle, AlertCircle } from 'lucide-react';

export interface WarehouseNodeData {
  name: string;
  location: string;
  region: string;
...
function WarehouseNode({ data, selected }: { data: WarehouseNodeData; selected?: boolean }) {
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className={`p-1.5 rounded ${colors.bg}`}>
          <Warehouse className={`w-5 h-5 ${colors.icon}`} />
        </div>
      ...
      <div className="space-y-1.5">
        {data.inventoryLevel !== undefined && (
          <div className="flex justify-between text-xs">
            <span className="text-gray-400">Inventory</span>
            <span className="text-white font-medium">{data.inventoryLevel}%</span>
          </div>
        )}
        {data.capacity !== undefined && (
          <div className="flex justify-between text-xs">
            <span className="text-gray-400">Capacity</span>
            <span className="text-white font-medium">{data.capacity}%</span>
          </div>
        )}
      ...
      <div className="absolute -top-2 -right-2 px-2 py-0.5 bg-cyan-600 rounded text-[10px] text-white font-medium">
        Warehouse
      </div>
