...
import { Truck, AlertTriangle, CheckCircle, AlertCircle } from 'lucide-react';

export interface DistributorNodeData {
  name: string;
  location: string;
  region: string;
...
type DistributorNodeType = Node<DistributorNodeData, 'distributor'>;

function DistributorNode({ data, selected }: { data: DistributorNodeData; selected?: boolean }) {
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className={`p-1.5 rounded ${colors.bg}`}>
          <Truck className={`w-5 h-5 ${colors.icon}`} />
        </div>
      ...
      <div className="space-y-1.5">
        {data.deliveryRate !== undefined && (
          <div className="flex justify-between text-xs">
            ...
            <span className="text-white font-medium">{data.deliveryRate}%</span>
          </div>
        )}
        {data.utilization !== undefined && (
          <div className="flex justify-between text-xs">
            <span className="text-gray-400">Utilization</span>
            <span className="text-white font-medium">{data.utilization}%</span>
          </div>
        )}
      ...
      <div className="absolute -top-2 -right-2 px-2 py-0.5 bg-orange-600 rounded text-[10px] text-white font-medium">
        Distributor
      </div>
...
export default memo(DistributorNode);
