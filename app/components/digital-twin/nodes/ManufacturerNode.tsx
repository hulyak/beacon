...
export interface ManufacturerNodeData {
  name: string;
  location: string;
  region: string;
...
type ManufacturerNodeType = Node<ManufacturerNodeData, 'manufacturer'>;

function ManufacturerNode({ data, selected }: { data: ManufacturerNodeData; selected?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1.5">
        {data.productionRate !== undefined && (
          <div className="flex justify-between text-xs">
            <span className="text-gray-400">Production</span>
            <span className="text-white font-medium">{data.productionRate}%</span>
          </div>
        )}
      ...
      <div className="absolute -top-2 -right-2 px-2 py-0.5 bg-purple-600 rounded text-[10px] text-white font-medium">
        Manufacturer
      </div>
