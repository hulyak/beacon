...
interface ConfidenceRequest {
  ...
  agentResults: {
    infoAgent?: AgentResult;
    scenarioAgent?: AgentResult;
    impactAgent?: AgentResult;
    strategyAgent?: AgentResult;
  };
  dataQuality?: {
    completeness: number;
    accuracy: number;
 ...
 */
http('calculateConfidence', async (req: Request, res: Response): Promise<void> => {
  // Handle CORS
...
function calculateAgentContributions(
  agentResults: ConfidenceRequest['agentResults'],
  analysisType: string
): ConfidenceResponse['agentContributions'] {
...
function calculateSingleAgentContribution(
  ...
  const adjustedConfidence = adjustConfidenceForPerformance(
    ...
    agentResult.errorRate,
    agentResult.dataPoints
  );
 ...
 */
function calculateAgentDataQuality(agentResult: AgentResult): number {
 ...
 * Adjust confidence based on performance metrics
 */
function adjustConfidenceForPerformance(
...
function calculateConfidenceBreakdown(
  agentContributions: ConfidenceResponse['agentContributions'],
  dataQuality: ConfidenceRequest['dataQuality'],
  context: ConfidenceRequest['context'],
  analysisType: string
): ConfidenceResponse['confidenceBreakdown'] {
  ...
  const uncertaintyAdjustment = calculateUncertaintyAdjustment(
    agentPerformance,
    dataQualityScore,
    contextualFactors,
    context
 ...
 */
function calculateWeightedAgentPerformance(
  agentContributions: ConfidenceResponse['agentContributions']
): number {
 ...
 * Calculate contextual factors score
 */
function calculateContextualFactorsScore(
  context: ConfidenceRequest['context'],
  analysisType: string
): number {
  ...
  if (context.region) {
    score += regionComplexity[context.region as keyof typeof regionComplexity] || 0;
  }
  ...
  if (context.severity) {
    score += severityComplexity[context.severity as keyof typeof severityComplexity] || 0;
  }
  ...
  if (context.scenarioType) {
    score += scenarioComplexity[context.scenarioType as keyof typeof scenarioComplexity] || 0;
  }
  ...
  score += analysisComplexity[analysisType as keyof typeof analysisComplexity] || 0;
 ...
 * Calculate uncertainty adjustment
 */
function calculateUncertaintyAdjustment(
  agentPerformance: number,
  dataQuality: number,
  contextualFactors: number,
  context: ConfidenceRequest['context']
  ...
  if (agentPerformance < 60) adjustment -= 10;
  if (dataQuality < 60) adjustment -= 8;
  if (contextualFactors < 60) adjustment -= 5;
...
function calculateOverallConfidence(
  breakdown: ConfidenceResponse['confidenceBreakdown']
): number {
...
function identifyUncertaintyFactors(
  ...
  context: ConfidenceRequest['context'],
  overallConfidence: number
): ConfidenceResponse['uncertaintyFactors'] {
  const factors: ConfidenceResponse['uncertaintyFactors'] = [];
 ...
 */
http('calculateConfidenceHealth', (req: Request, res: Response): void => {
  res.set('Access-Control-Allow-Origin', '*');
