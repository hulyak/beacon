...
interface ComprehensiveImpactResponse {
  ...
  financialImpact: FinancialImpactDetails;
  operationalImpact: OperationalImpactDetails;
  cascadeEffects: CascadeEffectDetails;
  kpiMetrics: KPIMetricsDetails;
...
interface FinancialImpactDetails {
  directCosts: {
    ...
    alternativeSupplierCosts: number;
    laborCosts: number;
  };
  indirectCosts: {
...
interface OperationalImpactDetails {
  deliveryDelays: {
    averageDelay: number;
    maxDelay: number;
...
interface CascadeEffectDetails {
  networkAnalysis: {
    totalAffectedNodes: number;
    criticalPathsImpacted: number;
    propagationDepth: number;
    networkResilience: number;
  };
  upstreamEffects: Array<{
    supplierId: string;
    impactLevel: number;
...
interface KPIMetricsDetails {
  performanceMetrics: {
    onTimeDelivery: { current: number; projected: number; impact: number };
    fillRate: { current: number; projected: number; impact: number };
    costEfficiency: { current: number; projected: number; impact: number };
    qualityMetrics: { current: number; projected: number; impact: number };
  };
  riskMetrics: {
    supplyRisk: number;
    demandRisk: number;
 ...
 */
export async function comprehensiveImpactAnalysis(req: Request, res: Response): Promise<void> {
  try {
    // Validate request
    if (req.method !== 'POST') {
    ...
    const request: ImpactAssessmentRequest = req.body;
 ...
 * Perform comprehensive impact analysis
 */
async function performComprehensiveAnalysis(request: ImpactAssessmentRequest): Promise<ComprehensiveImpactResponse> {
 ...
 */
async function calculateFinancialImpact(request: ImpactAssessmentRequest): Promise<FinancialImpactDetails> {
 ...
 */
async function calculateOperationalImpact(request: ImpactAssessmentRequest): Promise<OperationalImpactDetails> {
  const severityMultiplier = getSeverityMultiplier(request.severity);
 ...
 */
async function analyzeCascadeEffects(request: ImpactAssessmentRequest): Promise<CascadeEffectDetails> {
  const severityMultiplier = getSeverityMultiplier(request.severity);
 ...
 */
async function generateKPIMetrics(
  request: ImpactAssessmentRequest,
  financialImpact: FinancialImpactDetails,
  operationalImpact: OperationalImpactDetails
): Promise<KPIMetricsDetails> {
  const severityMultiplier = getSeverityMultiplier(request.severity);
...
function generateRecommendations(
  ...
  if (overallImpact.severity === 'critical') {
    recommendations.push(
      ...
      'Establish customer communication plan for delivery delays'
    );
  }
...
export { comprehensiveImpactAnalysis as default };