...
// Real-time monitoring service for VoiceOps AI
// Implements live data processing with sub-second latency requirements
...
interface MonitoringStream {
  streamId: string;
  metricType: 'delivery_performance' | 'cost_efficiency' | 'risk_level' | 'sustainability' | 'custom';
...
interface RealTimeMonitoringRequest {
  action: 'start_stream' | 'stop_stream' | 'get_current' | 'update_thresholds';
...
const RealTimeMonitoringRequestSchema = z.object({
  action: z.enum(['start_stream', 'stop_stream', 'get_current', 'update_thresholds']),
  streams: z.array(z.object({
    streamId: z.string(),
    metricType: z.enum(['delivery_performance', 'cost_efficiency', 'risk_level', 'sustainability', 'custom']),
    frequency: z.number().min(100).max(60000), // 100ms to 60s
    thresholds: z.object({
      warning: z.number(),
      critical: z.number(),
    }),
  ...
  timeWindow: z.number().min(1).max(3600).optional(), // 1s to 1h
  includeHistory: z.boolean().optional(),
});

// In-memory storage for active streams (in production, use Redis or similar)
const activeStreams = new Map<string, MonitoringStream>();
const streamData = new Map<string, MetricDataPoint[]>();
 ...
 */
export const realTimeMonitoring = async (req: Request, res: Response): Promise<void> => {
  try {
    // Enable CORS
 ...
 */
async function handleStartStream(streams: MonitoringStream[]) {
 ...
 */
async function handleStopStream(streamIds: string[]) {
 ...
 */
async function handleGetCurrent(streamIds?: string[], timeWindow: number = 300, includeHistory: boolean = false) {
 ...
 */
async function handleUpdateThresholds(streams: MonitoringStream[]) {
 ...
 */
function generateDataPoint(stream: MonitoringStream) {
 ...
 */
function checkThresholds(stream: MonitoringStream, dataPoint: MetricDataPoint) {
 ...
 */
function calculateAggregation(data: MetricDataPoint[], aggregation: string): number {
 ...
 */
function calculateTrend(data: MetricDataPoint[]): 'up' | 'down' | 'stable' {
 ...
 */
function getThresholdStatus(value: number, thresholds: { warning: number; critical: number }): 'normal' | 'warning' | 'critical' {
