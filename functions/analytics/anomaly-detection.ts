...
// Anomaly detection service for VoiceOps AI
// Implements threshold monitoring and pattern analysis for supply chain anomaly identification
...
interface PatternRule {
  ...
  name: string;
  description: string;
  pattern: 'spike' | 'drop' | 'trend' | 'oscillation' | 'flatline';
  parameters: {
    windowSize: number; // number of data points
    threshold: number; // percentage change or deviation
...
interface AnomalyDetectionRequest {
  action: 'detect_threshold' | 'detect_pattern' | 'comprehensive' | 'configure';
  data?: DataPoint[];
...
const AnomalyDetectionRequestSchema = z.object({
  action: z.enum(['detect_threshold', 'detect_pattern', 'comprehensive', 'configure']),
  ...
  patternRules: z.array(z.object({
    ruleId: z.string(),
    name: z.string(),
    description: z.string(),
    pattern: z.enum(['spike', 'drop', 'trend', 'oscillation', 'flatline']),
    parameters: z.object({
      windowSize: z.number().min(3).max(100),
  ...
  timeWindow: z.number().min(1).max(168).optional(), // 1 hour to 1 week
 ...
 */
export const anomalyDetection = async (req: Request, res: Response): Promise<void> => {
  try {
    // Enable CORS
 ...
 */
async function handleThresholdDetection(data: DataPoint[], thresholds: AnomalyThreshold[], sensitivity: string) {
  const anomalies = [];
  ...
  for (const [source, points] of Object.entries(groupedData)) {
    ...
    const threshold = enabledThresholds.find(t => 
      source.includes(t.metricName) || t.metricName === 'default'
    );
 ...
 */
async function handlePatternDetection(data: DataPoint[], patternRules: PatternRule[], sensitivity: string) {
  const anomalies = [];
 ...
 */
async function handleComprehensiveDetection(
  data: DataPoint[], 
  thresholds: AnomalyThreshold[], 
  patternRules: PatternRule[],
  timeWindow: number,
 ...
 */
async function handleConfiguration(thresholds: AnomalyThreshold[], patternRules: PatternRule[]) {
  return {
    success: true,
    configuration: {
      thresholds,
 ...
 */
function groupDataBySource(data: DataPoint[]): Record<string, DataPoint[]> {
 ...
 */
function checkThresholdAnomaly(point: DataPoint, threshold: AnomalyThreshold, sensitivity: string) {
 ...
 */
function detectPattern(points: DataPoint[], rule: PatternRule, source: string, sensitivity: string): any[] {
  const anomalies = [];
  const { windowSize, threshold, duration } = rule.parameters;
  const sensitivityMultiplier = { low: 0.8, medium: 1.0, high: 1.2 }[sensitivity] || 1.0;
  const adjustedThreshold = threshold * sensitivityMultiplier;

  if (points.length < windowSize) return anomalies;
 ...
 */
function detectSpecificPattern(window: DataPoint[], pattern: string, threshold: number) {
 ...
 */
function detectSpike(values: number[], threshold: number) {
 ...
 * Detect drop pattern
 */
function detectDrop(values: number[], threshold: number) {
  const baseline = values.slice(0, -2).reduce((sum, val) => sum + val, 0) / (values.length - 2);
  const recent = values.slice(-2);
 ...
 */
function detectTrend(values: number[], threshold: number) {
 ...
 */
function detectOscillation(values: number[], threshold: number) {
  let changes = 0;
  let totalChange = 0;
 ...
 */
function detectFlatline(values: number[], threshold: number) {
 ...
 */
function generateAnomalySummary(anomalies: any[]) {
 ...
 */
function deduplicateAnomalies(anomalies: any[]): any[] {
  const deduplicated = [];
  const seen = new Set();
 ...
 */
function generateImpactAssessment(anomalies: any[]) {
  ...
  else if (highCount > 0 || anomalies.length > 5) overallImpact = 'medium';
 ...
 */
function calculateThresholdImpact(value: number, threshold: AnomalyThreshold): string {
  ...
  if (relativeDeviation > 0.5) return 'high';
 ...
 */
function calculatePatternImpact(pattern: string, severity: string): string {
  ...
  const baseImpact = patternImpacts[pattern as keyof typeof patternImpacts] || 'medium';
  
  // Adjust based on severity
  if (severity === 'critical') return 'critical';
  if (severity === 'high' && baseImpact !== 'critical') return 'high';
 ...
 */
function getThresholdRecommendations(metricName: string, isUpper: boolean): string[] {
  const recommendations = {
  ...
  return recommendations[metricName as keyof typeof recommendations] || ['Review metric and take appropriate action'];
}

/**
 * Get pattern recommendations
 */
function getPatternRecommendations(pattern: string, source: string): string[] {
  const recommendations = {
  ...
  return recommendations[pattern as keyof typeof recommendations] || ['Monitor closely and investigate'];
}

/**
 * Validate thresholds configuration
 */
function validateThresholds(thresholds: AnomalyThreshold[]) {
  const errors = [];
 ...
 */
function validatePatternRules(patternRules: PatternRule[]) {
  const errors = [];
