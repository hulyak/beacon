...
// Regions
export const regions: SupplyChainRegion[] = [
  {
    id: 'asia',
    name: 'Asia',
...
export const activeAlerts: Alert[] = [
  ...
  },
  {
    ...
    timestamp: formatTimestamp(new Date(Date.now() - 8 * 60 * 60 * 1000)), // 8 hours ago
...
export function getRegionData(regionId: Region): SupplyChainRegion | undefined {
  return regions.find(r => r.id === regionId);
}

export function getSuppliersByRegion(regionId: Region): Supplier[] {
  return suppliers.filter(s => s.region === regionId);
}

export function getRiskFactorsByRegion(regionId: Region): RiskFactor[] {
  return riskFactors.filter(rf => rf.region === regionId);
}

export function getRiskFactorsByCategory(category: RiskCategory): RiskFactor[] {
  if (category === 'all') return riskFactors;
  return riskFactors.filter(rf => rf.category === category);
}

export function getAlertsByPriority(priority: Priority | 'all'): Alert[] {
  if (priority === 'all') return activeAlerts;
  return activeAlerts.filter(a => a.priority === priority);
}

export function getAlertsByRegion(regionId: Region): Alert[] {
  return activeAlerts.filter(a => a.region === regionId || a.region === 'global');
}

export function getScenarioTemplate(scenarioType: ScenarioType): ScenarioTemplate | undefined {
  return scenarioTemplates.find(st => st.type === scenarioType);
}

/**
 * Generate realistic risks based on region and category
 */
export function generateRisksForRegion(regionId: Region, category?: RiskCategory): Risk[] {
  ...
  categoryRiskFactors.forEach((riskFactor, index) => {
    const severity: Severity = 
      riskFactor.currentLevel >= 70 ? 'critical' :
...
function generateRiskDescription(riskFactor: RiskFactor, regionData?: SupplyChainRegion): string {
...
function generateRecommendations(riskFactor: RiskFactor, regionData?: SupplyChainRegion): string[] {
 ...
 */
export function calculateRegionRiskLevel(regionId: Region): Severity {
 ...
 */
export function getRegionRiskSummary(regionId: Region): string {
  const regionData = getRegionData(regionId);
  const risks = generateRisksForRegion(regionId);
...
export function calculateScenarioImpact(
  ...
  severity: string
): { estimatedCost: number; currency: string; timeframe: string } {
...
export function generateScenarioOutcomes(
  ...
  const outcomesByType: Record<ScenarioType, Outcome[]> = {
    supplier_failure: [
