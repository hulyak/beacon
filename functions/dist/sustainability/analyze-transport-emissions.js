...
exports.healthCheck = exports.analyzeTransportEmissions = void 0;
const zod_1 = require("zod");
const TransportEmissionsRequestSchema = zod_1.z.object({
    ...
    analysisType: zod_1.z.enum(['comparison', 'optimization', 'breakdown']),
    includeAlternatives: zod_1.z.boolean().optional(),
});
 ...
 */
const analyzeTransportEmissions = async (req, res) => {
    try {
        // Enable CORS
...
exports.analyzeTransportEmissions = analyzeTransportEmissions;
/**
 * Calculate emission efficiency score (0-100)
 */
function calculateEmissionEfficiency(mode, emissions, distance) {
 ...
 */
function calculateEcoFriendliness(mode) {
 ...
 */
function calculateAlternativeReduction(mode) {
 ...
 */
function generateModeComparison(routeAnalysis) {
 ...
 * Generate optimization recommendations
 */
function generateOptimizationRecommendations(routeAnalysis) {
 ...
 */
function calculateFeasibilityScore(route, alternativeMode) {
    const metrics = TRANSPORT_METRICS[alternativeMode];
    ...
    if (alternativeMode === 'air' && route.distance > 1000)
        score += 25;
    else if (alternativeMode === 'sea' && route.distance > 500)
        score += 25;
    else if (alternativeMode === 'rail' && route.distance > 200 && route.distance < 2000)
        score += 25;
    else if (alternativeMode === 'road' && route.distance < 1000)
        score += 25;
 ...
 */
function generateComparisonSummary(routeAnalysis) {
    ...
    return {
        ...
        mostEfficientMode: routeAnalysis.reduce((best, route) => route.efficiency.emissionEfficiency > (best?.efficiency?.emissionEfficiency || 0) ? route : best)?.mode || 'unknown',
        leastEfficientMode: routeAnalysis.reduce((worst, route) => route.efficiency.emissionEfficiency < (worst?.efficiency?.emissionEfficiency || 100) ? route : worst)?.mode || 'unknown',
    };
}
/**
 * Generate emissions breakdown
 */
function generateEmissionsBreakdown(routeAnalysis) {
    ...
    const totalEmissions = Object.values(breakdown).reduce((sum, mode) => sum + mode.emissions, 0);
 ...
 */
function generatePerformanceMetrics(routeAnalysis) {
    return {
 ...
 */
function calculatePotentialSavings(routeAnalysis, recommendations) {
...
exports.default = exports.analyzeTransportEmissions;
//# sourceMappingURL=analyze-transport-emissions.js.map