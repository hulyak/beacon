...
exports.healthCheck = exports.recommendGreenAlternatives = void 0;
const zod_1 = require("zod");
const GreenAlternativeRequestSchema = zod_1.z.object({
    currentRoutes: zod_1.z.array(zod_1.z.object({
        id: zod_1.z.string(),
        ...
        emissions: zod_1.z.number().min(0),
    })),
    optimizationGoals: zod_1.z.array(zod_1.z.enum(['emissions', 'cost', 'time', 'sustainability'])),
    constraints: zod_1.z.object({
        maxCostIncrease: zod_1.z.number().min(0).max(100).optional(),
        maxTimeIncrease: zod_1.z.number().min(0).max(100).optional(),
        requiredModes: zod_1.z.array(zod_1.z.enum(['air', 'sea', 'rail', 'road'])).optional(),
        excludedModes: zod_1.z.array(zod_1.z.enum(['air', 'sea', 'rail', 'road'])).optional(),
    }).optional(),
    includeMultiModal: zod_1.z.boolean().optional(),
});
 ...
 */
const recommendGreenAlternatives = async (req, res) => {
    try {
        // Enable CORS
        ...
        const { currentRoutes, optimizationGoals, constraints = {}, includeMultiModal = true } = validationResult.data;
        // Generate alternatives for each route
        const routeAlternatives = await Promise.all(currentRoutes.map(route => generateRouteAlternatives(route, optimizationGoals, constraints, includeMultiModal)));
        ...
        const potentialSavings = calculatePotentialSavings(currentRoutes, routeAlternatives, bestStrategy);
...
exports.recommendGreenAlternatives = recommendGreenAlternatives;
/**
 * Generate alternatives for a single route
 */
async function generateRouteAlternatives(route, goals, constraints, includeMultiModal) {
    const alternatives = [];
    // Single-mode alternatives
    const availableModes = Object.keys(GREEN_TRANSPORT_DATA).filter(mode => {
        if (constraints.excludedModes?.includes(mode))
            return false;
        if (constraints.requiredModes && !constraints.requiredModes.includes(mode))
            return false;
        if (mode === route.mode)
            return false;
        const modeData = GREEN_TRANSPORT_DATA[mode];
    ...
    if (includeMultiModal) {
        for (const multiModal of MULTIMODAL_OPTIONS) {
            if (multiModal.modes.some(mode => constraints.excludedModes?.includes(mode)))
                continue;
            if (constraints.requiredModes && !multiModal.modes.some(mode => constraints.requiredModes?.includes(mode)))
                continue;
 ...
 */
function calculateSingleModeAlternative(route, mode, goals) {
    const modeData = GREEN_TRANSPORT_DATA[mode];
 ...
 */
function calculateMultiModalAlternative(route, multiModal, goals) {
    ...
    const avgSustainabilityScore = multiModal.modes.reduce((sum, mode) => sum + GREEN_TRANSPORT_DATA[mode].sustainabilityScore, 0) / multiModal.modes.length;
    return {
 ...
 */
function meetsConstraints(alternative, originalRoute, constraints) {
 ...
 */
function calculateAlternativeScore(alternative, goals) {
    let score = 0;
 ...
 */
function calculateFeasibilityScore(route, mode, modeData) {
    ...
    if (modeData.infrastructure === 'excellent')
        score += 10;
    else if (modeData.infrastructure === 'good')
        score += 5;
    // Weather dependency (lower is better)
    if (modeData.weatherDependency === 'low')
        score += 10;
    else if (modeData.weatherDependency === 'high')
        score -= 10;
 ...
 */
function calculateMultiModalFeasibility(route, modes) {
 ...
 */
function calculateImplementationComplexity(currentMode, newMode) {
    ...
    return complexityMatrix[currentMode]?.[newMode] || 'medium';
}
/**
 * Identify risk factors
 */
function identifyRiskFactors(mode, modeData, route) {
    const risks = [];
    if (modeData.weatherDependency === 'high')
        risks.push('weather_delays');
    if (modeData.capacity === 'low')
        risks.push('capacity_constraints');
    if (route.distance > modeData.suitableDistances.max * 0.8)
        risks.push('distance_limitations');
    if (modeData.infrastructure !== 'excellent')
        risks.push('infrastructure_limitations');
    return risks;
}
/**
 * Identify benefits
 */
function identifyBenefits(route, mode, modeData) {
    const benefits = [];
 ...
 * Find best overall strategy
 */
function findBestOverallStrategy(routeAlternatives, goals) {
    const strategies = [];
    ...
    const balancedStrategy = {
        ...
        routes: routeAlternatives.map(ra => ra.alternatives.find((alt) => alt.feasibilityScore > 70) || ra.bestAlternative).filter(Boolean),
        totalEmissionReduction: 0,
        totalCostChange: 0,
        totalTimeChange: 0,
        feasibilityScore: 0,
    ...
    return strategies.sort((a, b) => {
        if (goals.includes('emissions'))
            return b.totalEmissionReduction - a.totalEmissionReduction;
 ...
 * Generate network-level recommendations
 */
function generateNetworkRecommendations(currentRoutes, routeAlternatives) {
    ...
    const routesByOrigin = currentRoutes.reduce((groups, route) => {
        if (!groups[route.origin])
            groups[route.origin] = [];
        groups[route.origin].push(route);
        return groups;
    }, {});
    const consolidationOpportunities = Object.values(routesByOrigin).filter((routes) => routes.length > 1);
 ...
 * Calculate potential savings
 */
function calculatePotentialSavings(currentRoutes, routeAlternatives, bestStrategy) {
    const currentTotals = {
    ...
    const optimizedTotals = bestStrategy.routes.reduce((totals, route, index) => {
 ...
 * Generate implementation roadmap
 */
function generateImplementationRoadmap(routeAlternatives, constraints) {
    const phases = [];
 ...
 */
function calculateAverageSustainabilityScore(routes) {
...
exports.default = exports.recommendGreenAlternatives;
//# sourceMappingURL=recommend-green-alternatives.js.map