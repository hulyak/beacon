 ...
 */
(0, functions_framework_1.http)('trackAgentContributions', async (req, res) => {
    // Handle CORS
    ...
    try {
        const { sessionId, analysisType, agentExecutions } = req.body;
        ...
        const agentContributions = calculateAgentContributions(agentExecutions, analysisType);
        // Calculate coordination metrics
        const coordinationMetrics = calculateCoordinationMetrics(agentExecutions);
        // Generate performance summary
        const performanceSummary = generatePerformanceSummary(agentContributions, agentExecutions);
        // Generate optimization recommendations
        const recommendations = generateOptimizationRecommendations(agentContributions, coordinationMetrics, agentExecutions);
        const response = {
            sessionId,
            agentContributions,
            coordinationMetrics,
 ...
 */
function calculateAgentContributions(agentExecutions, analysisType) {
    ...
    const weights = contributionWeights[analysisType] || contributionWeights.impact;
    return {
        infoAgent: calculateSingleAgentContribution(agentExecutions.infoAgent, weights.infoAgent, 'Information Gathering and Data Collection'),
        scenarioAgent: calculateSingleAgentContribution(agentExecutions.scenarioAgent, weights.scenarioAgent, 'Scenario Modeling and Simulation'),
        impactAgent: calculateSingleAgentContribution(agentExecutions.impactAgent, weights.impactAgent, 'Impact Assessment and Analysis'),
        strategyAgent: calculateSingleAgentContribution(agentExecutions.strategyAgent, weights.strategyAgent, 'Strategy Optimization and Recommendations')
    };
}
/**
 * Calculate contribution for a single agent
 */
function calculateSingleAgentContribution(execution, weight, agentRole) {
    if (!execution) {
        return {
            confidence: 0,
            processingTime: 0,
 ...
 */
function calculateDataQuality(execution) {
 ...
 */
function adjustConfidenceForExecution(execution) {
 ...
 */
function calculateCoordinationMetrics(agentExecutions) {
    const executions = Object.values(agentExecutions).filter(Boolean);
 ...
 */
function calculateParallelEfficiency(executions) {
 ...
 */
function calculateDataConsistency(executions) {
 ...
 */
function calculateAgentSynchronization(executions) {
    if (executions.length <= 1)
        return 100;
 ...
 * Generate performance summary
 */
function generatePerformanceSummary(agentContributions, agentExecutions) {
 ...
 * Generate optimization recommendations
 */
function generateOptimizationRecommendations(agentContributions, coordinationMetrics, agentExecutions) {
    const recommendations = [];
    // Performance recommendations
    const agents = Object.entries(agentContributions);
    const lowPerformingAgents = agents.filter(([_, agent]) => agent.confidence < 70);
    if (lowPerformingAgents.length > 0) {
        recommendations.push(`Improve performance for ${lowPerformingAgents.map(([name, _]) => formatAgentName(name)).join(', ')} by reviewing data sources and processing algorithms.`);
    }
    // Coordination recommendations
    if (coordinationMetrics.parallelEfficiency < 70) {
        recommendations.push('Optimize agent coordination to improve parallel execution efficiency. Consider adjusting agent start times and dependencies.');
    }
    if (coordinationMetrics.dataConsistency < 80) {
        recommendations.push('Improve data consistency across agents by standardizing data sources and validation procedures.');
    }
    // Processing time recommendations
    const slowAgents = agents.filter(([_, agent]) => agent.processingTime > 45000);
    if (slowAgents.length > 0) {
        recommendations.push(`Optimize processing time for ${slowAgents.map(([name, _]) => formatAgentName(name)).join(', ')} through caching, algorithm optimization, or resource scaling.`);
    }
    // Data quality recommendations
    const lowQualityAgents = agents.filter(([_, agent]) => agent.dataQuality < 75);
    if (lowQualityAgents.length > 0) {
        recommendations.push(`Enhance data quality for ${lowQualityAgents.map(([name, _]) => formatAgentName(name)).join(', ')} by improving data validation and source reliability.`);
    }
    // Error handling recommendations
    const executions = Object.values(agentExecutions).filter(Boolean);
    const totalErrors = executions.reduce((sum, e) => sum + e.errorCount, 0);
    if (totalErrors > 0) {
        recommendations.push('Implement enhanced error handling and retry mechanisms to reduce execution failures.');
    }
    // Default recommendation if no issues found
    if (recommendations.length === 0) {
        recommendations.push('Agent performance is optimal. Continue monitoring for consistency and consider A/B testing new optimization strategies.');
    }
 ...
 */
(0, functions_framework_1.http)('trackAgentContributionsHealth', (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    if (req.method === 'OPTIONS') {
        res.status(204).send('');
        return;
...
//# sourceMappingURL=track-agent-contributions.js.map