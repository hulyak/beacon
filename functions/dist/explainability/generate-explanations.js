 ...
 */
(0, functions_framework_1.http)('generateExplanations', async (req, res) => {
    // Handle CORS
    ...
    try {
        const { recommendationId, analysisType, explanationType = 'summary', context = {} } = req.body;
        ...
        const reasoning = generateStepByStepReasoning(recommendationId, analysisType, context);
        // Calculate overall confidence
        const confidence = calculateOverallConfidence(reasoning, analysisType);
        // Identify uncertainty factors
        const uncertaintyFactors = identifyUncertaintyFactors(analysisType, context, confidence);
        // Generate natural language explanation
        const explanation = generateNaturalLanguageExplanation(reasoning, analysisType, explanationType, context);
        // Create voice-optimized description
        const voiceDescription = generateVoiceDescription(explanation, reasoning, confidence);
        const response = {
            explanation,
            reasoning,
            confidence,
 ...
 */
function generateStepByStepReasoning(recommendationId, analysisType, context) {
    ...
    const baseSteps = reasoningTemplates[analysisType] || reasoningTemplates.impact;
 ...
 */
function adjustConfidenceForContext(baseConfidence, context, analysisType) {
    ...
    if (context.region) {
        const adjustment = regionAdjustments[context.region] || 0;
    ...
    if (context.severity) {
        const adjustment = severityAdjustments[context.severity] || 0;
        adjustedConfidence += adjustment;
    }
    ...
    adjustedConfidence += complexityAdjustments[analysisType] || 0;
    return Math.max(50, Math.min(95, adjustedConfidence));
}
/**
 * Contextualize reasoning with specific scenario details
 */
function contextualizeReasoning(baseReasoning, context) {
    ...
    if (context.region) {
        ...
        const regionContext = regionContexts[context.region];
        if (regionContext) {
    ...
    if (context.scenarioType) {
        ...
        const scenarioContext = scenarioContexts[context.scenarioType];
        if (scenarioContext) {
 ...
 */
function calculateOverallConfidence(reasoning, analysisType) {
 ...
 */
function identifyUncertaintyFactors(analysisType, context, confidence) {
    const factors = [];
 ...
 * Generate natural language explanation
 */
function generateNaturalLanguageExplanation(reasoning, analysisType, explanationType, context) {
    const analysisTypeNames = {
        impact: 'Impact Assessment',
        sustainability: 'Sustainability Analysis',
        optimization: 'Strategy Optimization'
    };
    const analysisName = analysisTypeNames[analysisType] || 'Analysis';
 ...
 */
function generateSummaryExplanation(reasoning, analysisName, context) {
 ...
 */
function generateDetailedExplanation(reasoning, analysisName, context) {
 ...
 */
function generateDecisionTreeExplanation(reasoning, analysisName, context) {
 ...
 */
function generateVoiceDescription(explanation, reasoning, confidence) {
 ...
 */
(0, functions_framework_1.http)('generateExplanationsHealth', (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    if (req.method === 'OPTIONS') {
        res.status(204).send('');
        return;
...
//# sourceMappingURL=generate-explanations.js.map