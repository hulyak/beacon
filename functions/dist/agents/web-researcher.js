...
exports.WebResearcherAgent = void 0;
const base_agent_1 = require("./base-agent");
const tavily_client_1 = require("../shared/tavily-client");
class WebResearcherAgent extends base_agent_1.BaseAgent {
    constructor(geminiClient) {
        super(geminiClient);
        this.name = 'web-researcher';
        this.description = 'Gathers real-time supply chain intelligence from web sources';
        this.capabilities = [
            'web_search',
            'news_monitoring',
        ...
        this.systemPrompt = `You are a Supply Chain Intelligence Researcher. Your role is to:
...
- Flag high-impact developments requiring immediate attention`;
        ...
        if ((0, tavily_client_1.isTavilyConfigured)()) {
            try {
                this.tavilyClient = (0, tavily_client_1.getTavilyClient)();
            }
            catch {
    ...
    async process(input) {
        ...
        try {
            ...
            const supplier = input.parameters?.supplier;
            const searchType = input.parameters?.searchType || 'comprehensive';
            let risks = [];
            let news = [];
            let geopoliticalAlerts = [];
            ...
            // Generate AI summary
            const summary = await this.generateSummary(input.query, risks, news, geopoliticalAlerts);
            const result = {
            ...
            const confidence = this.calculateConfidence(this.tavilyClient !== null, risks.length, news.length, geopoliticalAlerts.length);
            ...
            return this.createErrorOutput(error instanceof Error ? error : 'Web research failed', startTime);
        }
    }
    /**
     * Generate AI-powered summary of research findings
     */
    async generateSummary(query, risks, news, alerts) {
...
`;
        try {
            const summary = await this.callGemini(`Summarize the following web research findings in 2-3 concise sentences, highlighting the most important developments:\n\n${context}`);
            return summary;
        }
        catch {
     ...
     * Calculate confidence based on data quality
     */
    calculateConfidence(hasTavily, riskCount, newsCount, alertCount) {
     ...
     */
    generateFollowUpSuggestions(region, supplier, risks) {
        const suggestions = [];
        if (region) {
        ...
        return [
            {
                ...
                summary: 'Real-time web intelligence service not configured. Using internal monitoring data.',
     ...
     */
    getFallbackGeopoliticalAlerts() {
        return [
            {
                id: 'fallback_geo_1',
                title: 'Geopolitical monitoring active',
                summary: 'Configure Tavily API for real-time geopolitical intelligence. Using baseline monitoring.',
                regions: ['Global'],
                severity: 'low',
                source: 'beacon-internal',
                url: '',
...
exports.WebResearcherAgent = WebResearcherAgent;
