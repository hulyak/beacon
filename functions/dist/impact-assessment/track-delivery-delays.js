 ...
 */
(0, functions_framework_1.http)('trackDeliveryDelays', async (req, res) => {
    // Handle CORS
    ...
    try {
        const { scenarioType, region, severity, timeHorizon = 60 } = req.body;
 ...
 */
function calculateDeliveryDelays(scenarioType, region, severity) {
    ...
    const basePattern = baseDelayPatterns[scenarioType]
        || baseDelayPatterns.supplier_failure;
    const severityMultiplier = severityMultipliers[severity] || 1.0;
    const complexityMultiplier = regionalComplexity[region] || 1.0;
    const baseOrderVolume = regionalOrderVolumes[region] || 3000;
 ...
 */
function generateTimelineProjection(baseDelays, scenarioType, severity, timeHorizon) {
    const projection = [];
    ...
    const pattern = recoveryPatterns[scenarioType]
 ...
 */
function calculateImpactMetrics(deliveryDelays, scenarioType, region) {
    ...
    const getCancellationRate = (delay) => {
        if (delay <= 7)
            return 0.05; // 5% cancellation for delays up to 1 week
        if (delay <= 14)
            return 0.12; // 12% cancellation for delays up to 2 weeks
        if (delay <= 21)
            return 0.25; // 25% cancellation for delays up to 3 weeks
        if (delay <= 35)
            return 0.40; // 40% cancellation for delays up to 5 weeks
        return 0.60; // 60% cancellation for longer delays
    };
    const avgOrderValue = averageOrderValues[region] || 950;
    ...
    const baseRecoveryTime = baseRecoveryTimes[scenarioType] || 28;
 ...
 */
(0, functions_framework_1.http)('trackDeliveryDelaysHealth', (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    if (req.method === 'OPTIONS') {
        res.status(204).send('');
        return;
...
//# sourceMappingURL=track-delivery-delays.js.map