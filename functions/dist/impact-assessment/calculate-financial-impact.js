...
const analytics_schemas_1 = require("../../lib/validation/analytics-schemas");
/**
 ...
 */
(0, functions_framework_1.http)('calculateFinancialImpact', async (req, res) => {
    // Handle CORS
    ...
    try {
        // Validate request
        const validation = analytics_schemas_1.ImpactAssessmentRequestSchema.safeParse(req.body);
        ...
        const financialImpact = calculateFinancialImpactByScenario(scenarioType || 'supplier_failure', region || 'asia', severity || 'moderate');
        // Calculate operational impact
        const operationalImpact = calculateOperationalImpact(scenarioType || 'supplier_failure', severity || 'moderate');
        // Generate mitigation strategies
        const recommendations = generateMitigationStrategies(scenarioType || 'supplier_failure', financialImpact.totalImpact);
        // Calculate confidence based on data quality and scenario complexity
        const confidence = calculateConfidenceScore(scenarioType || 'supplier_failure', region || 'asia');
        const response = {
            financialImpact,
            operationalImpact,
            recommendations,
        ...
        // Validate response
        const responseValidation = analytics_schemas_1.ImpactAssessmentResponseSchema.safeParse(response);
 ...
 */
function calculateFinancialImpactByScenario(scenarioType, region, severity) {
    ...
    const baseImpact = baseImpacts[scenarioType] || baseImpacts.supplier_failure;
 ...
 */
function calculateOperationalImpact(scenarioType, severity) {
    const severityMultipliers = {
        minor: 0.5,
        moderate: 1.0,
        severe: 1.8,
    ...
    const baseDelay = baseDelays[scenarioType] || baseDelays.supplier_failure;
 ...
 */
function generateMitigationStrategies(scenarioType, totalImpact) {
    const strategies = {
        supplier_failure: [
            {
 ...
 */
function calculateConfidenceScore(scenarioType, region) {
    ...
    const baseConfidence = scenarioConfidence[scenarioType] || 75;
    const regionAdjustment = regionAdjustments[region] || 0;
    return Math.max(50, Math.min(95, baseConfidence + regionAdjustment));
}
//# sourceMappingURL=calculate-financial-impact.js.map