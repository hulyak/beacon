 ...
 */
(0, functions_framework_1.http)('analyzeCascadeEffects', async (req, res) => {
    // Handle CORS
    ...
    try {
        const { scenarioType, originNode, region, severity } = req.body;
        ...
        const cascadeAnalysis = analyzeCascadePropagation(networkTopology, origin, scenarioType, severity || 'moderate');
        const response = {
 ...
 */
function generateNetworkTopology(region) {
 ...
 */
function getDefaultOriginNode(scenarioType, region) {
 ...
 */
function analyzeCascadePropagation(networkTopology, originNodeId, scenarioType, severity) {
    ...
    const multiplier = severityMultipliers[severity] || 0.6;
    // Find origin node
    ...
    while (queue.length > 0) {
        ...
        if (visited.has(node.id) || depth > 3)
            continue; // Limit cascade depth
        visited.add(node.id);
 ...
 */
function findConnectedNodes(node, networkTopology) {
    ...
    return networkTopology.filter(n => n.id !== node.id &&
 ...
 */
function calculateImpactDecay(fromNode, toNode, depth) {
    // Base decay factors
 ...
 */
function getTypeRelationshipMultiplier(fromType, toType) {
    const relationships = {
        supplier: {
 ...
 * Calculate overall network impact score
 */
function calculateNetworkImpactScore(affectedNodes, totalNodes) {
    if (totalNodes === 0)
        return 0;
 ...
 */
function getNodeWeight(nodeType) {
    const weights = {
    ...
    return weights[nodeType] || 0.5;
}
//# sourceMappingURL=analyze-cascade-effects.js.map