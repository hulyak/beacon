...
// Multi-Criteria Analysis Service
...
interface StrategyScore {
  cost: number; // 0-100 (higher is better - lower cost)
  risk: number; // 0-100 (higher is better - lower risk)
  sustainability: number; // 0-100 (higher is better)
  timeline: number; // 0-100 (higher is better - faster implementation)
  quality: number; // 0-100 (higher is better)
  feasibility: number; // 0-100 (higher is better)
}
...
interface MultiCriteriaRequest {
  strategies: Strategy[];
  weights: CriteriaWeights;
  preferences: {
    riskTolerance: 'conservative' | 'moderate' | 'aggressive';
    timeHorizon: 'short' | 'medium' | 'long'; // <1yr, 1-3yr, >3yr
    budgetConstraint: number;
    sustainabilityPriority: 'low' | 'medium' | 'high';
  };
}

interface RankedStrategy extends Strategy {
  analysis: {
...
interface MultiCriteriaResponse {
  ...
  analysis: {
    ...
    tradeoffAnalysis: {
      costVsRisk: string;
      timeVsSustainability: string;
      qualityVsFeasibility: string;
    };
    sensitivityAnalysis: {
      criteriaImpact: {
        [key: string]: number; // impact on ranking if weight changes by 10%
      };
    };
  };
  recommendations: {
...
export const performMultiCriteriaAnalysis = async (req: Request, res: Response): Promise<void> => {
  try {
    const request: MultiCriteriaRequest = req.body;
    ...
    const rankedStrategies: RankedStrategy[] = request.strategies.map(strategy => {
      const criteriaBreakdown: { [key: string]: { score: number; weight: number; weightedValue: number } } = {};
      let weightedScore = 0;

      // Calculate weighted score for each criterion
      Object.entries(strategy.scores).forEach(([criterion, score]) => {
        const weight = adjustedWeights[criterion as keyof CriteriaWeights];
      ...
      if (strategy.scores.cost > 80) strengths.push('Excellent cost efficiency');
      else if (strategy.scores.cost < 40) weaknesses.push('High implementation cost');

      if (strategy.scores.risk > 80) strengths.push('Low risk profile');
      else if (strategy.scores.risk < 40) weaknesses.push('High risk factors');

      if (strategy.scores.sustainability > 80) strengths.push('Strong environmental benefits');
      else if (strategy.scores.sustainability < 40) weaknesses.push('Limited sustainability impact');

      if (strategy.scores.timeline > 80) strengths.push('Quick implementation');
      else if (strategy.scores.timeline < 40) weaknesses.push('Extended implementation timeline');

      if (strategy.scores.quality > 80) strengths.push('High quality outcomes');
      else if (strategy.scores.quality < 40) weaknesses.push('Quality concerns');

      if (strategy.scores.feasibility > 80) strengths.push('High implementation feasibility');
      else if (strategy.scores.feasibility < 40) weaknesses.push('Implementation challenges');
...
function applyPreferenceAdjustments(weights: CriteriaWeights, preferences: any): CriteriaWeights {
  ...
  Object.keys(adjusted).forEach(key => {
    adjusted[key as keyof CriteriaWeights] /= total;
  });

  return adjusted;
}

function generateTopStrategyReasoning(strategy: RankedStrategy, weights: CriteriaWeights): string {
  ...
  topCriteria.forEach(criterion => {
    const score = strategy.scores[criterion as keyof StrategyScore];
    if (score > 70) {
...
function generateTradeoffAnalysis(strategies: RankedStrategy[]): any {
  const avgScores = {
...
function performSensitivityAnalysis(strategies: RankedStrategy[], weights: CriteriaWeights): any {
  const criteriaImpact: { [key: string]: number } = {};

  Object.keys(weights).forEach(criterion => {
    // Calculate ranking change if this criterion weight increases by 10%
    const adjustedWeights = { ...weights };
    adjustedWeights[criterion as keyof CriteriaWeights] *= 1.1;
    
    // Renormalize
    const total = Object.values(adjustedWeights).reduce((sum, weight) => sum + weight, 0);
    Object.keys(adjustedWeights).forEach(key => {
      adjustedWeights[key as keyof CriteriaWeights] /= total;
    });
    ...
    const newRanking = strategies.map(strategy => {
      ...
      Object.entries(strategy.scores).forEach(([crit, score]) => {
        newScore += score * adjustedWeights[crit as keyof CriteriaWeights];
      });
...
function generateRecommendations(strategies: RankedStrategy[], preferences: any): any {
  ...
  const alternatives = top3.slice(1).map(strategy => 
    `Consider ${strategy.name} as an alternative (score: ${strategy.analysis.weightedScore}/100)`
  );
